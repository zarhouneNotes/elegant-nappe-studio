...
import type { FirebaseMessage } from "@/services/firebaseService";
...
const sampleMessages: FirebaseMessage[] = [
  {
    ...
    message: "I'm interested in bulk orders for my restaurant. Can you provide a custom quote?",
    read: false,
  },
];

export default function AdminInbox() {
  const [messages, setMessages] = useState<FirebaseMessage[]>(sampleMessages);
  const [selected, setSelected] = useState<string | null>(null);

  const markRead = (id: string) => {
    setMessages((prev) => prev.map((m) => (m.id === id ? { ...m, read: true } : m)));
    setSelected(id);
    // TODO: await markMessageRead(id);
  };
          ...
          {messages.map((m) => (
            <button
              ...
              className={`w-full text-left rounded-lg border p-4 transition-colors ${
                selected === m.id ? "border-primary bg-primary/5" : "border-border hover:bg-accent"
              }`}
            >
