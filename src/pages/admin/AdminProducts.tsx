...
// Local state placeholder until Firebase is configured
const sampleProducts: FirebaseProduct[] = [
  {
    id: "1",
    title: "Classic White Cotton Nappe",
    description: "A timeless white cotton tablecloth.",
    images: [],
    colors: ["White", "Beige"],
    dimensions: ["140 x 180 cm", "140 x 220 cm"],
    category: "everyday",
    featured: true,
  },
];
...
export default function AdminProducts() {
  const [products, setProducts] = useState<FirebaseProduct[]>(sampleProducts);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editing?.id) {
      setProducts((prev) => prev.map((p) => (p.id === editing.id ? { ...product, id: editing.id } : p)));
      toast.success("Product updated");
      // TODO: await updateProduct(editing.id, product);
    } else {
      const newId = Date.now().toString();
      setProducts((prev) => [{ ...product, id: newId }, ...prev]);
      toast.success("Product added");
      // TODO: await addProduct(product);
    }
    resetForm();
  };
  ...
  const toggleFeatured = (id: string) => {
    ...
    // TODO: await updateProduct(id, { featured: !current });
  };
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-lg border border-border p-6">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={resetForm} className="rounded-md border border-border px-6 py-2 text-sm font-medium text-muted-foreground hover:bg-accent">
              Cancel
            </button>
          </div>
        </form>
      ...
      <div className="mt-6 space-y-3">
        {products.map((p) => (
          <div key={p.id} className="flex items-center justify-between rounded-lg border border-border p-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => toggleFeatured(p.id!)} className="rounded p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground" title="Toggle featured">
